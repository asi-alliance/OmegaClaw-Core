...
def load_telegram(monkeypatch, auth_enabled=True):
    ...
    auth.get_channel_saved_group_id = lambda channel, group: (channel, str(group)) in saved
    auth.store_channel_authenticated_group_id = lambda channel, group: saved.add((channel, str(group))) or True

    def authenticate_channel_group(channel, group, candidate=None):
        if candidate == "secret":
            auth.store_channel_authenticated_group_id(channel, group)
            return "auth_bound"
        return "allow" if auth.get_channel_saved_group_id(channel, group) else "ignore"

    auth.authenticate_channel_group = authenticate_channel_group
...
def test_reply_uses_chat_that_supplied_the_next_message(monkeypatch):
...
def test_group_authentication_allows_every_member_of_that_group(monkeypatch):
    telegram = load_telegram(monkeypatch)

    assert telegram._is_allowed_message("group", "2", "hello") == "ignore"
    assert telegram._is_allowed_message("group", "2", "auth secret") == "auth_bound"
    assert telegram._is_allowed_message("group", "3", "hello") == "allow"
    assert telegram._is_allowed_message("other-group", "3", "hello") == "ignore"
